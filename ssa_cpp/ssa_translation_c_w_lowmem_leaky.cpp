...
//	N = 10;
	...
	MatrixXi leaky_probe_matrix(N_rib,N);	
	leaky_probe_matrix.setZero(N_rib,N);
    ...
    while( t < tf)
    {
		...
		if (NR > old_NR){
			...
			for(int i=0; i < n_probes; i++){
				...
				if ( r1 > 1-k_probe){
					leaky_probe_matrix(NR-1,probe_loc[i]) = 1;
				}
			}
  ...
  //      std::cout << "-------------" << std::endl;
        ...
        while( (it<=Nt-1) && (t>t_array[it])) {
			...
			for(int i=0; i < NR; i++){
				...
				intensity += leaky_probe_matrix.block(i,0,1,X(0,i)-1).sum();

				
			}
			
		
            X_array(it) = intensity;
		...
		if (Sn.row(ind-1).sum() < 0){
			...
			leaky_probe_matrix.block(0,0,NR,N) = leaky_probe_matrix.block(1,0,NR+1,N);
