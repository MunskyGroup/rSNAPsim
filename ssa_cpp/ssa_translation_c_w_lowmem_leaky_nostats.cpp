...
void translationSSA(double* kelong, double* t_array, int Nt, double kbind, double kcompl, int* SSA_intensity, int N, int FRAP, int Inhibitor, double inhibit_time, int seed, int fNt, int* frap_result, int* x0, int r_footprint, int* SSA_probe, double* k_probe,int* probe_loc,int* n_probes,int Ncolor)
{
	...
	rib_times - an array to be filled with the times of a Nribs to wat/ch how long they take to go through the simulation
...
//	N = 10;
	...
	MatrixXi leaky_probe_matrix(Ncolor*N_rib,N);	
    ...
    while( t < tf)
    {
		...
		if (NR > old_NR){
			...
			for (int k=0; k < total_probes*2; k+=2){
    			...
    			//std::cout << "probe loc color: " <<probe_loc[k] << std::endl;
    			//std::cout << "probe loc loc: " <<probe_loc[k+1] << std::endl;
			...
			for (int j=0; j < Ncolor; j++){
    			...
    			std::cout << "nprobes: " << n_probes[j] << std::endl;
			
    			for(int i=0; i < n_probes[j]; i++){
    				r1 = ((double) rand()/ (RAND_MAX));
    				std::cout << "probe loc: " <<probe_loc[i,j] << std::endl;
    				if ( r1 > 1-k_probe){
    					leaky_probe_matrix(Ncolor*(NR-1) + j ,probe_loc[j,i]) = 1;
    				}
    			}
    		}
			std::cout << "adding probes: " << leaky_probe_matrix.block(0,0,NR+3,25)  << std::endl;
...
/* 		if((t > inhibit_time) && (t < inhibit_time+30)){
  ...
  //      std::cout << "-------------" << std::endl;
		...
		if (Sn.row(ind-1).sum() < 0){
			...
			//std::cout << "deleting leaky probe after" << leaky_probe_matrix.block(0,0,Ncolor*NR,25) << std::endl;
